...
import ru.argustelecom.ops.inf.queue.api.QueueManager;
import ru.argustelecom.ops.inf.queue.api.QueueManagerStatus;
import ru.argustelecom.ops.inf.queue.impl.task.DaemonQueueTask;
import ru.argustelecom.ops.inf.queue.impl.task.StandardQueueTask;
...
public class QueueManagerImpl implements QueueManager, QueueManagerCallback, ManagedTaskListener {
	...
	protected void postConstruct() {
		if (isAutoRunnable()) {
			log.info("Автоматический запуск обработчика очереди: ops.queue.autorunnable == true");
			startup();
		}
	}
	...
	private static final String AUTO_RUNNABLE = "ops.queue.autorunnable";
