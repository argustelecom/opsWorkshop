...
public class CustomerTypeDirectoryIT extends AbstractWebUITest {

    @Test
    public void shouldCreateCustomerType(
            @InitialPage CustomerTypeDirectoryPage page,
            @DataProvider(
                    providerClass = PartyTypeProvider.class,
                    contextPropertyName = PartyTypeProvider.PARTY_TYPE_NAME
            ) String partTypeName
    ) {
        String nameVal = uniqueId("Тестовый тип клиента");
        String descriptionVal = "Тестовое описание";

        page.openCreationDialog.click();
        page.customerCategory.select(COMPANY.getName());

        page.creationDialog.name.input(nameVal);
        page.creationDialog.partyType.select(partTypeName);
        ...
        assertEquals(partTypeName, page.partyType.getValue());
