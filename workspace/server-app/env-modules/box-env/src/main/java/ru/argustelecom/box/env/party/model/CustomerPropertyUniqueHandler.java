...
@DomainService
public class CustomerPropertyUniqueHandler extends BaseTypePropertyUniqueHandler {

	@Override
	public void handleEnable(@Observes @MakeUnique(typeClass = CustomerType.class, mode = ENABLE) UniqueEvent event) {
		super.handleEnable(event);
	}

	@Override
	public void handleDisable(@Observes @MakeUnique(typeClass = CustomerType.class, mode = DISABLE) UniqueEvent event) {
		super.handleDisable(event);
	}

	@Override
	protected List<Class<? extends TypeInstance<?>>> getInstanceClasses() {
		return of(CustomerTypeInstance.class);
	}
}
