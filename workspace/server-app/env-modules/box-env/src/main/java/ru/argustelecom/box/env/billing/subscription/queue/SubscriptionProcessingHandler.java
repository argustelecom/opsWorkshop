...
@QueueHandlerBean
@Named(value = SubscriptionProcessingHandler.HANDLER_NAME)
public class SubscriptionProcessingHandler implements QueueHandler {

	public static final String HANDLER_NAME = "subscriptionProcessingHandler";
	...
	public QueueHandlingResult handleWork(QueueEvent event) throws Exception {
		...
		processingSvc.processSubscription(subscription, new Date());
	...
	public static String genQueueName(Subscription subscription) {
		return "REGULAR_" + new EntityConverter().convertToString(subscription);
	}
