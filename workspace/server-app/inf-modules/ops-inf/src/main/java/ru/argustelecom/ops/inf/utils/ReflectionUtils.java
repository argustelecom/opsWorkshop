package ru.argustelecom.ops.inf.utils;
...
public final class ReflectionUtils {
	...
	public static <T extends Annotation> T extractAnnotation(Class<?> checkClass, Class<T> annotationClass) {
		T annotation = checkNotNull(checkClass).getAnnotation(annotationClass);
		if (annotation == null) {
			throw new SystemException(format("Class %s does not have annotation %s", checkClass.getName(),
					checkNotNull(annotationClass).getName()));
		}
		return annotation;
	}
}
