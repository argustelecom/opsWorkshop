...
@LifecycleBean
public class MustHaveAnyEntryInExtension implements LifecycleCdiValidator<ContractState, ContractExtension> {
	...
	public void validate(ExecutionCtx<ContractState, ? extends ContractExtension> ctx, ValidationResult<Object> result) {
		...
		if (extension.getEntries().isEmpty() && extension.getExcludedEntries().isEmpty()) {
			...
			result.errorv(extension, messages.contractExtensionEntriesAreNotSpecified());
		}
	}
