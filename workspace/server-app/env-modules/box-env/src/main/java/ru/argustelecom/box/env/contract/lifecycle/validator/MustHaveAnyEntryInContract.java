...
@LifecycleBean
public class MustHaveAnyEntryInContract implements LifecycleCdiValidator<ContractState, Contract> {
	...
	public void validate(ExecutionCtx<ContractState, ? extends Contract> ctx, ValidationResult<Object> result) {
		...
		if (contract.getEntries().isEmpty()) {
			...
			result.errorv(contract, messages.contractEntriesAreNotSpecified());
		}
	}
